...
<style>
/* =============================================
   CSS VARIABLES — Korean palette
   Merah gochujang + krem hanji + emas muted
============================================= */
:root{
  --red:       #c0392b;
  --red-light: #e8534a;
...
/* =============================================
   BACKGROUND — foto makanan Korea
============================================= */
body::before{
  content:'';
  position:fixed; inset:0; z-index:0;
  background: url('../assets/bg-korean.jpg') center/cover no-repeat fixed;
  /* gelapi supaya teks tetap terbaca, sedikit kemerahan */
  filter: brightness(0.28) saturate(1.3) sepia(0.15);
  pointer-events:none;
}

/* overlay gradient agar bagian bawah lebih gelap & nyaman dibaca */
body::after{
  content:'';
  position:fixed; inset:0; z-index:0;
  background: linear-gradient(
    160deg,
    rgba(26,15,7,0.55) 0%,
    rgba(80,20,10,0.35) 40%,
    rgba(26,15,7,0.65) 100%
  );
  pointer-events:none;
}
...
.order-type{
  display:flex; gap:0;
  background: rgba(183,144,85,0.12);
  border-radius:16px; padding:5px;
  margin-bottom:28px; width:fit-content;
  border:1px solid var(--border);
  animation: fadeDown 0.5s ease 0.08s both;
}
.type-btn{
  padding:11px 32px;
...
.item-icon{
  ...
  background:linear-gradient(135deg, var(--red-glow), rgba(183,144,85,0.15));
  display:flex; align-items:center; justify-content:center;
...
.item-info{ flex:1; }
.item-info h3{
  font-size:14px; font-weight:700; color:var(--dark); margin-bottom:2px;
}
.item-info p{
...
.plus{
  background:linear-gradient(135deg, var(--red), var(--red-light));
  color:white; box-shadow:0 3px 10px var(--red-glow);
}
...
.note-area textarea{
  ...
  font-family:'Plus Jakarta Sans', sans-serif;
  font-size:13px; color:#fff8f0; resize:none; height:70px;
  transition:border-color 0.2s;
}
.note-area textarea::placeholder{ color:rgba(255,220,170,0.4); }
...
.table.occupied{ background:#fff0ee; color:#ccc; cursor:not-allowed; border-color:#fcd5d1; }
...
.time-row input[type="time"]{
  ...
  font-family:'Plus Jakarta Sans', sans-serif;
  font-size:13px; color:#fff8f0; transition:0.2s;
  color-scheme: dark;
}
...
.pay-option.active{
  border-color:var(--red); background:rgba(192,57,43,0.18);
...
/* =============================================
   KOREAN DECO ELEMENT — 태극 inspired dots
============================================= */
.korean-deco{
  position:fixed; bottom:30px; left:30px; z-index:0;
  display:flex; flex-direction:column; gap:6px; opacity:0.15;
  pointer-events:none;
}
.korean-deco span{
...
<!-- Korean decorative dots (taegeuk inspired) -->
<div class="korean-deco">
    ...
    <div class="container">
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          ?>
          <div class="item">
            <div class="item-icon"><?= $emojis[$ei++ % count($emojis)] ?></div>
            <div class="item-info">
              <h3><?= $item['nama'] ?></h3>
