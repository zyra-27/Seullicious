...
.nav-item{
  ...
  color:rgba(255,240,210,0.6);text-decoration:none;
  font-size:13.5px;margin-bottom:3px;
  transition:background 0.2s, color 0.2s, transform 0.15s;
}
.nav-item:hover{background:rgba(255,255,255,0.08);color:rgba(255,240,210,0.9);transform:translateX(2px);}
...
<div class="sidebar" id="sidebar">
  ...
  <a href="pos.php" class="nav-item active"><i class="fas fa-utensils"></i> <?= $lang==='en'?'Menu':'Menu' ?></a>
  <a href="history.php" class="nav-item"><i class="fas fa-clock-rotate-left"></i> <?= $lang==='en'?'History':'Riwayat' ?></a>
  <a href="review_buka.php" class="nav-item"><i class="fas fa-star"></i> <?= $lang==='en'?'Review':'Ulasan' ?></a>
  <div class="sidebar-bottom">
    <div class="user-info" style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:rgba(255,255,255,0.06);border-radius:12px;margin-bottom:8px;">
      <div class="user-av" style="width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,#8b5a1a,#c28b3c);display:flex;align-items:center;justify-content:center;color:white;font-size:12px;font-weight:800;overflow:hidden;flex-shrink:0;">
        <?php if($photo_path): ?>
...
<div class="main" id="main">
  ...
  <div class="topbar">
    ...
    <div class="topbar-right">
      <div class="search-box">
        ...
        <input type="text" id="searchMenu" placeholder="<?= $lang==='en' ? 'Search menu...' : 'Cari menu...' ?>">
      </div>
      <!-- Language switcher -->
  ...
  <!-- CATEGORY TABS -->
  <div class="cat-bar">
    <button class="cat-tab active" onclick="filterCat('all',this)"><?= $lang==='en'?'All':'Semua' ?></button>
    <button class="cat-tab" onclick="filterCat('food',this)"><?= $lang==='en'?'Food':'Makanan' ?></button>
    <button class="cat-tab" onclick="filterCat('drink',this)"><?= $lang==='en'?'Drink':'Minuman' ?></button>
    <button class="cat-tab" onclick="filterCat('snack',this)"><?= $lang==='en'?'Snack':'Camilan' ?></button>
  </div>
  ...
  <div class="menu-area">
    ...
    <div class="popular-section">
      <div class="popular-top">
        <div class="popular-top-left">
          <div class="popular-fire"><i class="fas fa-fire"></i></div>
          <div>
            <div class="popular-title"><?= $lang==='en'?'Most <span>Popular</span>':'Paling <span>Populer</span>' ?></div>
            <div class="popular-subtitle-text"><?= $lang==='en'?'Most ordered menu items':'Menu paling banyak dipesan' ?></div>
          </div>
        </div>
        <div class="popular-badge-pill"><i class="fas fa-star"></i> <?= $lang==='en'?'Best Seller':'Terlaris' ?></div>
      </div>
      ...
      <div class="popular-grid">
        ...
        <div class="popular-card" style="background:<?= $t['bg'] ?>;">
          <div class="pop-rank"><?= $rankNum ?></div>
          <?php if($rank === 1): ?><div class="pop-hot-badge"><i class="fas fa-fire"></i> #1 Today</div><?php endif; ?>
          ...
          <div class="popular-card-img-wrap">
            <?php if(!empty($gambar)): ?>
              ...
              <div class="popular-card-img-placeholder" style="display:none;background:<?= $t['bg'] ?>"><i class="fas fa-utensils" style="color:rgba(255,255,255,0.4);font-size:28px;"></i></div>
            <?php else: ?>
              <div class="popular-card-img-placeholder" style="background:<?= $t['bg'] ?>"><i class="fas fa-utensils" style="color:rgba(255,255,255,0.4);font-size:28px;"></i></div>
            <?php endif; ?>
          </div>
          ...
          <div class="popular-card-body">
            <div>
              ...
              <div class="pop-orders-row" style="color:<?= $t['accent'] ?>">
                <i class="fas fa-fire" style="font-size:9px;"></i> <?= $total ?> <?= $lang==='en'?'times ordered':'kali dipesan' ?>
              </div>
            </div>
    ...
    <!-- DIVIDER -->
    <div class="section-divider"><span><i class="fas fa-list" style="color:#c28b3c;margin-right:6px;"></i><?= $lang==='en'?'All Menu':'Semua Menu' ?></span></div>
